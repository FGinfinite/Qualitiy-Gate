    ...
    for step in range(1, n_select):
        ...
        for candidate in range(n_total):
            ...
            candidates_info.append({
                'index': candidate,
                'min_distance': min_distance_to_selected,
                'is_best': False
            })
...
def visualize_fps_static(distance_matrix, selected_indices, steps, coords_2d, 
                        sample_ids=None, save_path=None):
    """创建FPS算法的静态可视化"""
    n_total = distance_matrix.shape[0]
    ...
    scatter = ax2.scatter(coords_2d[:, 0], coords_2d[:, 1], 
                         c='lightgray', s=100, alpha=0.6, label='未选择')
    
    # 按选择顺序绘制已选点
    colors = plt.cm.rainbow(np.linspace(0, 1, n_select))
    for i, idx in enumerate(selected_indices):
        ax2.scatter(coords_2d[idx, 0], coords_2d[idx, 1], 
                   c=[colors[i]], s=150, marker='*', 
                   edgecolors='black', linewidth=2,
                   label=f'第{i}个选择')
        
        # 标注选择顺序
        ax2.annotate(f'{i}', (coords_2d[idx, 0], coords_2d[idx, 1]), 
    ...
    ax5.text(0.05, 0.95, step_text, transform=ax5.transAxes, fontsize=10,
...
"""
    ...
    plt.suptitle(f'最远点采样(FPS)算法可视化 - 从{n_total}个样本中选择{n_select}个', 
                 fontsize=16, y=0.98)
    plt.tight_layout()
...
def create_fps_animation(distance_matrix, steps, coords_2d, save_path=None):
    ...
    text_box = ax1.text(0.02, 0.98, '', transform=ax1.transAxes, fontsize=12,
    ...
    def animate(frame):
        ...
        return unselected_scatter, selected_scatter, candidate_scatter, best_candidate_scatter, distance_line, current_point, text_box
...
def main():
    ...
    parser.add_argument('--projection-method', choices=['mds', 'pca'], default='mds', 
                       help='2D投影方法 (默认: mds)')
    ...
    fig_static = visualize_fps_static(distance_matrix, fps_selected, fps_steps, coords_2d,
                                     sample_ids, save_path_static)
    plt.show()
